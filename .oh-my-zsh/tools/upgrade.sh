...
if git pull --rebase --stat origin master; then
  printf "${BLUE}%s${RESET}\n" "Updating Oh My Zsh Custom Plugins and Themes"

  for custom in $(find $ZSH_CUSTOM/plugins $ZSH_CUSTOM/themes -type d -depth 1); do
    printf "${YELLOW}%s${RESET}\n" "Updating $(echo $custom | cut -d '/' -f 7)"
    cd "$custom" && git pull --rebase --stat origin master
  done
